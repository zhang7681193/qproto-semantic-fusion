...
foreach ($Policy in "variance","coverage","random") {
    foreach ($Seed in 0,1,2,3,4) {
        Invoke-KeyPolicyRun `
            -Out "runs\chop_policy_pennylane_highorder_$($Policy)_k8_seed$Seed" `
...
foreach ($Policy in "variance","coverage","random") {
    foreach ($Seed in 0,1,2) {
        Invoke-KeyPolicyRun `
            -Out "runs\chop_policy_mnist10_$($Policy)_k96_seed$Seed" `
        ...
        Invoke-KeyPolicyRun `
            -Out "runs\chop_policy_cifar10_$($Policy)_k96_seed$Seed" `
            -DatasetPath "data\cifar10_feat_resnet18_sz32_pt.npz" `
            -Backend "synthetic" `
            -NClasses 10 `
            -NTrain 12000 `
            -NTest 3000 `
...
foreach ($Policy in "variance","coverage","random") {
    foreach ($Seed in 0,1,2) {
        Invoke-KeyPolicyRun `
            -Out "runs\chop_policy_qiskit_aer_$($Policy)_k10_seed$Seed" `
