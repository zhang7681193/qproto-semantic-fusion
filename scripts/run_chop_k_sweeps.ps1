...
foreach ($K in 8,16,24,32,48) {
    foreach ($Seed in 0,1,2) {
        Invoke-ChopSweep `
            -Out "runs\chop_sweep_pennylane_highorder_k$($K)_seed$Seed" `
...
foreach ($K in 4,6,8,10,12) {
    foreach ($Seed in 0,1,2) {
        Invoke-ChopSweep `
            -Out "runs\chop_sweep_qiskit_aer_k$($K)_seed$Seed" `
...
foreach ($Dataset in $FullDatasets) {
    foreach ($K in 32,64,96,128) {
        foreach ($Seed in 0,1,2) {
            Invoke-ChopSweep `
                -Out "runs\chop_sweep_$($Dataset.Name)_k$($K)_seed$Seed" `
