...
foreach ($Dataset in $FullDatasets) {
    foreach ($Seed in 0,1,2) {
        Invoke-MaskedHopRun `
            -Out "runs\masked_hop_$($Dataset.Name)_seed$Seed" `
...
foreach ($Seed in 0,1,2) {
    Invoke-MaskedHopRun `
        -Out "runs\masked_hop_qiskit_aer_seed$Seed" `
...
foreach ($Seed in 0,1,2) {
    Invoke-MaskedHopRun `
        -Out "runs\masked_hop_pennylane_mnist4_pca_seed$Seed" `
...
foreach ($Seed in 0,1,2) {
    Invoke-MaskedHopRun `
        -Out "runs\masked_hop_pennylane_private_signal_seed$Seed" `
