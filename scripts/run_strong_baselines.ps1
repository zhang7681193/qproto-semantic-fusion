...
function Invoke-StrongBaselineRun {
    param(
        [string]$Out,
        [string]$DatasetPath,
        [string]$Backend,
        ...
        [int]$Seed,
        [double]$HopWeight,
        [string]$Methods = "fedavg_forced,fedprox_forced,fedadam_forced,fedavg_schema,fedprox_schema,fedadam_schema,fedproto_forced,fedproto_schema,qproto_cproto,qproto_chop,qproto_masked_hop",
        [string]$CommonPolicy = "random",
        [double]$ServerLr = 0.12,
        [double]$FedProtoMu = 0.1
    )
...
foreach ($Dataset in $FullDatasets) {
    foreach ($Seed in 0,1,2) {
        Invoke-StrongBaselineRun `
            -Out "runs\strong_baseline_$($Dataset.Name)_seed$Seed" `
...
foreach ($Seed in 0,1,2) {
    Invoke-StrongBaselineRun `
        -Out "runs\strong_baseline_qiskit_aer_seed$Seed" `
...
foreach ($Seed in 0,1,2) {
    Invoke-StrongBaselineRun `
        -Out "runs\strong_baseline_pennylane_highorder_seed$Seed" `
